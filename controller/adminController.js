...
exports.getStudentsByBatch = asyncHandler(async (req, res) => {
    ...
    res.status(200).json({ message: "bach wise Student Fetch Success", result })
})
...
exports.addAttendance = asyncHandler(async (req, res) => {
    const x = req.body.map(item => {
        return { studId: item.studId, isPresent: item.isPresent, date: item.date }
    })
    const result = await Attendance.findOne({ studId: x[0].studId, date: x[0].date })
    if (result) {
        res.status(400).json({ message: "Duplicate Attendance" })
    }
